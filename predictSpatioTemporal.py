...
# We expect people to predict the next num_predicted_frames frames.
#
...
#
# Main contributors: Isabelle Guyon, Lisheng Sun, January-May 2017
...
def predictSpatioTemporal(step_num, input_dir, output_dir, code_dir, \
                          ...
                          time_budget = 600, max_samples = 0, \
                          AR_order = 1, I_order = 0, MA_order = 0, \
                          num_predicted_frames=11, \
                          save_model = False, cache_data = False, \
                          cache_dir = "", skip_training=True, \
                          version = "linear_kernels" ):   
    ...
    path.append (os.path.join(code_dir, 'utilities'))
    ...
    if step_num == 0 and (not skip_training):
