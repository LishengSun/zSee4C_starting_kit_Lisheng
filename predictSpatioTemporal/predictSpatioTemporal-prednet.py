...
#IMPORTANT: This determines whether we train or not (debug_mode 1 => no training)
DEBUG_MODE=1
...
def predictSpatioTemporal(step_num, input_dir, output_dir, code_dir, \
                          ext = '.h5', verbose=True, debug_mode=1, \
...
#    from model import Model              # example model implementing persistence
    ...
    # Prednet initializations, added by Julio and modified by Isabelle
    data_io.mkdir("~/.keras")
    ...
    hyper_param = { 'cache_dir': cache_dir,
                    ...
                    'S_HELD': 101,
                    'S_PRED': 8,
                    'TRAIN_SAMPLES': [750, 800],
                    'test_batch_size': 10, # what's the difference between these batch sizes?
                    'train_batch_size': 4,
                    'nb_epoch': 150,
    ...
    if step_num == 0:
        if debug_mode == 0:
           ...
           M.fit(input_dir) # <==Isabelle: call training method
